<?php
// admin/modules/boletines/index.php
...
if (isset($_GET['mensaje'])) {
    if ($_GET['mensaje'] == 'creado') { $mensaje = 'Boletín creado exitosamente'; }
    elseif ($_GET['mensaje'] == 'editado') { $mensaje = 'Boletín actualizado exitosamente'; }
    elseif ($_GET['mensaje'] == 'eliminado') { $mensaje = 'Boletín eliminado exitosamente'; }
}

$items = $pdo->query("SELECT * FROM boletines ORDER BY fecha_publicacion DESC")->fetchAll();
?>
<!DOCTYPE html>
<html lang="es">
<head>
    ...
    <title>Boletines - Panel de Administración</title>
...
<div class="sidebar" style="background: #ffffff !important; background-color: #ffffff !important; box-shadow: 2px 0 15px rgba(0,0,0,0.06) !important; border-right: 1px solid #f0f0f0 !important;">
    ...
    <nav class="nav flex-column">
        ...
        <a class="nav-link active" style="color: #e0020d !important; background: #fff5f5 !important; border-left: 3px solid #e0020d !important;" href="index.php"><i class="fas fa-file-pdf"></i> <span>Boletines</span></a>
...
<div class="main-content">
    <div class="table-container">
        <div class="table-header">
            <h4><i class="fas fa-file-pdf"></i> Boletines</h4>
            <a href="crear.php" class="btn-add"><i class="fas fa-plus"></i> Nuevo Boletín</a>
        </div>
        <?php if ($mensaje): ?>
        ...
        <div class="table-responsive">
            <table class="table">
                <thead>
                    <tr><th>#</th><th>Número</th><th>Resumen</th><th>Fecha</th><th>PDF</th><th>Acciones</th></tr>
                </thead>
                <tbody>
                    <?php if (count($items) > 0): ?>
                        <?php foreach ($items as $i => $item): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><strong><?= htmlspecialchars($item['numero_boletin']) ?></strong></td>
                                <td><?= htmlspecialchars(substr($item['resumen'] ?? '', 0, 50)) ?>...</td>
                                <td><?= date('d/m/Y', strtotime($item['fecha_publicacion'])) ?></td>
                                <td><?= $item['archivo_pdf'] ? '<a href="../../../'.$item['archivo_pdf'].'" target="_blank"><i class="fas fa-file-pdf text-danger"></i></a>' : '-' ?></td>
                                <td>
                        ...
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="6" class="text-center">No hay boletines</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
