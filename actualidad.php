<?php
// actualidad.php - Pagina de Actualidad (Noticias) con paginacion
...
$stmt = $pdo->prepare("SELECT * FROM noticias ORDER BY fecha_publicacion DESC LIMIT ? OFFSET ?");
...
<head>
    ...
    <title>Actualidad - Dialogo y Desarrollo Peru</title>
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <p style="color:#999; margin-top:10px;">
                    <?= $total_noticias ?> noticia<?= $total_noticias != 1 ? 's' : '' ?> en total
                </p>
            </div>
        </div>
    </div>
                ...
                <?php foreach ($noticias as $noticia): ?>
                    <div class="col-lg-4 col-md-6">
                        <div class="grids5-info">
                            <a href="<?= obtenerLink($noticia) ?>" class="card-image" <?= targetBlank($noticia) ?>>
                                <img src="<?= htmlspecialchars(obtenerImagenNoticia($noticia)) ?>" alt="<?= htmlspecialchars($noticia['titulo']) ?>" onerror="this.src='assets/images/noticias/nota-facebook-21-11-25.png'" />
                            </a>
                            <div class="card-content">
                                <div class="card-date"><?= fechaMes($noticia['fecha_publicacion']) ?></div>
                                <h4 class="card-title">
                                    <a href="<?= obtenerLink($noticia) ?>" <?= targetBlank($noticia) ?>>
                                        <?= htmlspecialchars($noticia['titulo']) ?>
                                    </a>
                                </h4>
                                <a href="<?= obtenerLink($noticia) ?>" class="card-link" <?= targetBlank($noticia) ?>>
                                    Leer &rarr;
                                </a>
                            </div>
                        </div>
